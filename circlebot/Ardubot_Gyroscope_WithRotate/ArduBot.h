...
struct moveRobotOutputs moveRobot(int x, int y, int z, float gyro=0) { //taking x, y, (-1000 to 1000), and gyro (in milliradians, 0 to 360,000) values, runs the motors at speeds to move robot in correct direction
  struct motorspeeds speeds;
  struct axis axis;
  ...
  if (fabs(gyro) < 1){
    ...
    xp = x * cosgyro - y * singyro; // rotating vectors
    yp = x * singyro + y * cosgyro; // with respect to gyro angle  
  }
  else{
    xp = x;
    yp = y;
