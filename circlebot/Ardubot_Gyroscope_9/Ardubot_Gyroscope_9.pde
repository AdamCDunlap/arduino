...
void loop(){ 
  ...
  if (1800 > pulseLeftX_filt  && pulseLeftX_filt  > 800 && // Only go if each stick is within range
      1700 > pulseLeftY_filt  && pulseLeftY_filt  > 900 &&
      1900 > pulseRightX_filt && pulseRightX_filt > 800){
...
//  Serial.flush();
  ...
  struct moveRobotOutputs moveRobotOutputs = moveRobot(strafe, forward, rotate, AR.angle);
  ...
  if(millis() - previousSerialTime > 200){
    ...
    Serial.write((unsigned char*z)&moveRobotOutputs.speeds.speed3, 2);
