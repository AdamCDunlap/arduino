...
void loop(){ 
  ...
  unsigned int pulseLeftX = joyTimeLX(); //get pulse times
...
//  Serial.flush();
  ...
  if(millis() - previousSerialTime > 200){
    #ifdef labview
    Serial.write((byte*)&AR.angle, 4); //Sends angle as binary data
    Serial.write((byte*)&AR.rate, 4); //Sends rate as binary data
